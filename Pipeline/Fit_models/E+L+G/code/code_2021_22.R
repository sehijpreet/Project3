...
getwd()
setwd("../2021-22")
setwd('../Desktop/PhD data/P3/Pipeline/Fit_models/E+L+G/output/2021-22')
...
# --- Prediction set: 2021-22 ---
pred_idx  <- which(Y[, colSEA] == "2021-22")
train_idx <- which(Y[, colSEA] != "2021-22")
...
# --- Output folder for 2021-22 predictions ---
dir.create("2021-22_pred", showWarnings = FALSE)
...
for(trait in trait_names){
  ...
  # 2021-22 predictions only
  pred_out <- all_preds[pred_idx, ]
  write.csv(pred_out,  paste0("2021-22_pred/", trait, "_2021-22.csv"), row.names = TRUE)
...
# Combined 2021-22 all traits
pred_combined <- data.frame(Season = Y[pred_idx, colSEA], Genotype = Y[pred_idx, colVAR])

for(trait in trait_names){
  tmp <- read.csv(paste0("2021-22_pred/", trait, "_2021-22.csv"))
  pred_combined[, trait] <- tmp$y_hat
}

write.csv(pred_combined,"2021-22_pred/ELG_all_traits_combined.csv", row.names = FALSE)
