...
rm(list=ls())
getwd()
setwd('../Desktop/PhD data/P3/Pipeline/Fit_models/E+L+G/output/2024-25')
...
# --- Prediction set: 2024-25 ---
pred_idx  <- which(Y[, colSEA] == "2024-25")
train_idx <- which(Y[, colSEA] != "2024-25")
...
# --- Output folder for 2024-25 predictions ---
dir.create("2024-25_pred", showWarnings = FALSE)
...
for(trait in trait_names){
  ...
  # 2024-25 predictions only
  pred_out <- all_preds[pred_idx, ]
  write.csv(pred_out,  paste0("2024-25_pred/", trait, "_2024-25.csv"), row.names = TRUE)
...
# Combined 2024-25 all traits
pred_combined <- data.frame(Season = Y[pred_idx, colSEA], Genotype = Y[pred_idx, colVAR])

for(trait in trait_names){
  tmp <- read.csv(paste0("2024-25_pred/", trait, "_2024-25.csv"))
  pred_combined[, trait] <- tmp$y_hat
}

write.csv(pred_combined,"2024-25_pred/ELG_all_traits_combined.csv", row.names = FALSE)
