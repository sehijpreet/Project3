...
getwd()
setwd("../2022-23")
#setwd('../Desktop/PhD data/P3/Pipeline/Fit_models/E+L+G/output/2022-23')
...
# --- Prediction set: 2022-23 ---
pred_idx  <- which(Y[, colSEA] == "2022-23")
train_idx <- which(Y[, colSEA] != "2022-23")
...
# --- Output folder for 2022-23 predictions ---
dir.create("2022-23_pred", showWarnings = FALSE)
...
for(trait in trait_names){
  ...
  # 2022-23 predictions only
  pred_out <- all_preds[pred_idx, ]
  write.csv(pred_out,  paste0("2022-23_pred/", trait, "_2022-23.csv"), row.names = TRUE)
...
# Combined 2022-23 all traits
pred_combined <- data.frame(Season = Y[pred_idx, colSEA], Genotype = Y[pred_idx, colVAR])

for(trait in trait_names){
  tmp <- read.csv(paste0("2022-23_pred/", trait, "_2022-23.csv"))
  pred_combined[, trait] <- tmp$y_hat
}

write.csv(pred_combined,"2022-23_pred/ELG_all_traits_combined.csv", row.names = FALSE)
