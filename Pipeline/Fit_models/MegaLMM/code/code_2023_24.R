...
Y2_tr <- Y2
Y2_tr[Y2_tr$Season=='2023-24',-(1:2)] <- NA
...
saveRDS(MegaLMM_state,"MegaLMM_state_2023-24.rds")

pred_out <- cbind(Y2[test,c("Season","Genotype")], as.data.frame(pred))
write.csv(pred_out,"MegaLMM_predictions_2023-24.csv",row.names=FALSE)

pred_all <- cbind(Y2[test,c("Season","Genotype")],as.data.frame(U_hat[test,]))
write.csv(pred_all,"MegaLMM_all_predictions_2023-24.csv",row.names=FALSE)

accuracy_out <- data.frame(Trait=colnames(Y_tr)[cum],Correlation=accuracy)
write.csv(accuracy_out,"MegaLMM_accuracy_2023-24.csv",row.names=FALSE)
