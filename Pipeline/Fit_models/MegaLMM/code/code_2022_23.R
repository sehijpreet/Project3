...
Y2_tr <- Y2
Y2_tr[Y2_tr$Season=='2022-23',-(1:2)] <- NA
...
MegaLMM_state <- setup_model_MegaLMM(Y_tr, ~1+(1|ID), data=sample_data,
                                     relmat=list(ID=G), run_parameters=run_parameters, run_ID="MegaLMM_2023_24")
...
saveRDS(MegaLMM_state,"MegaLMM_state_2022-23.rds")

pred_out <- cbind(Y2[test,c("Season","Genotype")], as.data.frame(pred))
write.csv(pred_out,"MegaLMM_predictions_2022-23.csv",row.names=FALSE)

pred_all <- cbind(Y2[test,c("Season","Genotype")],as.data.frame(U_hat[test,]))
write.csv(pred_all,"MegaLMM_all_predictions_2022-23.csv",row.names=FALSE)

accuracy_out <- data.frame(Trait=colnames(Y_tr)[cum],Correlation=accuracy)
write.csv(accuracy_out,"MegaLMM_accuracy_2022-23.csv",row.names=FALSE)
