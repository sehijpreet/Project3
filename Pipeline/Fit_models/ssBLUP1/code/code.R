...
run_a <- function(a){
  ...
  for(trait in traits){
    ...
    fm  <- BGLR(y=yNA, ETA=ETA, nIter=12000, burnIn=2000,
                saveAt=paste0(td,"/"), verbose=FALSE)

    acc <- cor(y[tr], fm$yHat[tr], use="complete.obs")

    out <- data.frame(Season=Res$Season, Genotype=Res$Genotype,
                      y_obs=y, y_hat=fm$yHat)
    rownames(out) <- uid

    write.csv(out[pr,], paste0("alpha_", a, "/pred/", trait, ".csv"), row.names=TRUE)

    res_a[[trait]] <- list(trait=trait, alpha=a, cor=round(acc,3),
                           varE=round(fm$varE,4),
                           varH=round(fm$ETA[[3]]$varU,4))
...
all_res <- mclapply(alphas, run_a, mc.cores=6
)
