...
# Combined 2025-26 all traits
pred_combined <- data.frame(Season = Y[pred_idx, colSEA], Genotype = Y[pred_idx, colVAR])

for(trait in trait_names){
  tmp <- read.csv(paste0("2025-26_pred/", trait, "_2025-26.csv"))
  pred_combined[, trait] <- tmp$y_hat
}

write.csv(pred_combined,"2025-26_pred/ELGP_orig_all_traits_combined.csv", row.names = FALSE)
