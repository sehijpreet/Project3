...
# Run from: P3/Pipeline/Fit_models/P_pred/Across_season
...
for(target in targets){
  ...
  outdir <- file.path(HERE,target,"output")
  ...
  Y <- as.matrix(rs[,grep("_cum$",names(rs))])
  rownames(Y) <- id
  ...
  if(!any(keep)) stop("No usable predicted phenomic traits for target ",target)
  ...
  if(!any(keepR)) stop("No usable residual phenomic traits for target ",target)
...
cat("\nAll across-season analyses finished.\n")
